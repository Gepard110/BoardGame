...
public class GinScript : MonoBehaviour {
    ...
    void Create() {
        Vector3 pos = this.transform.position;
        if (Mathf.Abs(pos.y - ClickedObject.transform.position.y) == 1 && Mathf.Abs(pos.x - ClickedObject.transform.position.x) == 0 && Vector2.Distance(pos, ClickedObject.transform.position) != 0) {
