...
public class KinScript : MonoBehaviour {
    ...
    void Move() {
        if (Input.GetMouseButtonDown(0)) {
            ...
            Vector3 pos = this.transform.position;
            if (Vector2.Distance(pos, ClickedObject.transform.position) <= 2*Mathf.Sqrt(2) && Vector2.Distance(pos, ClickedObject.transform.position) != 0) {
